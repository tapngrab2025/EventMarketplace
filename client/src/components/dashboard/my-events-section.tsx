    ...
    <div className="space-y-6">
      {events?.length === 0 ? (
        <div className="rounded-[18px] border border-dashed border-slate-300 bg-slate-50/80 px-6 py-12 text-center">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-primaryGreen/10 text-primaryGreen">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
            Create your first event to start adding stalls, products, and
            coupons.
          </p>
          {enableButton && (
            onAddEvent ? (
              <Button
                type="button"
                onClick={onAddEvent}
                className="mt-5 rounded-xl bg-slate-950 hover:bg-primaryGreen"
              >
              ...
              <Link to="/vendor/events/new">
                <Button className="mt-5 rounded-xl bg-slate-950 hover:bg-primaryGreen">
                  <Plus className="h-4 w-4" />
                  Add Event
                </Button>
              </Link>
        ...
        events?.map((event) => {
          ...
          return (
            ...
            >
              <div className="grid gap-0 xl:grid-cols-[minmax(0,0.92fr)_minmax(0,1.35fr)]">
                <div className="relative min-h-[260px] overflow-hidden bg-slate-100 xl:min-h-full">
                  <img
                ...
                <div className="flex flex-col p-5 sm:p-6">
                    ...
                    {enableButton && (
                      onEditEvent ? (
                        <Button
                          ...
                          className="w-fit shrink-0 rounded-xl border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
                        >
                  ...
                  <div className="mt-5 grid gap-3 sm:grid-cols-2">
                    ...
                    <InfoTile
                      icon={Store}
                      label="Stalls"
                      value={`${eventStalls.length} active`}
                    />
                    <InfoTile
                      icon={Package}
                      label="Products"
              ...
              <div className="border-t border-slate-100 bg-slate-50/70 p-5 sm:p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h4 className="text-lg font-bold text-slate-950">Stalls</h4>
                  </div>
                  {enableButton && (
                    onAddStall ? (
                      <Button
                        ...
                        size="sm"
                        className="w-fit rounded-xl bg-slate-950 hover:bg-primaryGreen"
                      >
                        <Plus className="h-4 w-4" />
                        Add Stall
                      </Button>
                ...
                {eventStalls.length === 0 ? (
                  <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-8 text-center text-sm text-slate-500">
                    No stalls added to this event yet.
                  </div>
                ) : (
                  <div className="mt-4 space-y-4">
                    {eventStalls.map((stall) => {
                      ...
                      const canManageStall = stall.vendorId === user?.id;
...
function InfoTile({
  ...
  return (
    <div className="flex min-h-[76px] items-start gap-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4">
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-primaryGreen shadow-sm">
