...
export default function EventDetailsCategoryPage() {
    const { addToCart } = useCart();
    const [, params] = useRoute("/event/category/:category");
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-white to-teal-50 relative overflow-hidden">
            ...
            <div className="absolute top-1/2 left-1/4 w-64 h-64 bg-teal-30 opacity-5 rounded-full"></div>
                ...
                {!events || events.length === 0 ? (
                    <div className={`min-h-[70vh] flex flex-col items-center justify-center p-4 transition-all duration-700 ${animateItems ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                        <div className="text-center space-y-6 bg-white p-10 rounded-xl shadow-xl max-w-md">
                            ...
                            <h1 className="text-4xl font-bold text-primaryGreen">No Events in {category}</h1>
                            <p className="text-gray-600">We couldn't find any events in this category at the moment.</p>
                    ...
                    <>
                        <div className={`transition-all duration-700 ${animateItems ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
                            <h1 className="text-4xl font-bold text-primaryGreen mb-8">Events in {category}</h1>
                        </div>
                        ...
                        {events.map((event, eventIndex) => (
                            <div key={event.id} className="mb-12">
                                            ...
                                            <div className="relative">
                                                ...
                                                <Link to={`/event/${event.id}`}>
                                                    <img
                                                        src={event.imageUrl}
                                                        alt={event.name}
                                                        className="w-full h-64 lg:h-[400px] object-cover transition-transform duration-700 hover:scale-105 cursor-pointer"
                                                    />
                                                </Link>
                                                <div className="absolute bottom-0 left-0 right-0 p-6 z-20">
                                                    <Link to={`/event/${event.id}`}>
                                                        <CardTitle className="text-3xl font-bold text-white mb-2 hover:text-primaryOrange transition-colors">{event?.name}</CardTitle>
                                                    </Link>
                                ...
                                <div className={`mt-8 transition-all duration-700 delay-300 ${animateItems ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                                    ...
                                    <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                        {event?.products?.filter(product => product?.products?.approved)?.map((product, index) => (
                                            <div 
                                                ...
                                                style={{ transitionDelay: `${400 + index * 50 + eventIndex * 200}ms` }}
                                            >
